...
#nastaveni ctverecku
    ...
    def update_colors(self):
        ...
        #pokud je cislo vetsi jak 4, barva cisla se zmeni na jinou
        if self.number > 4:
    ...
    def is_deadlocked(self):
        for x, y in all_cells():
            ...
            if self.can_merge(x + 1, y, number) or self.can_merge(x, y + 1, number):
                return False
        return True
#generovani novych ctverecku na volna mista, pokud nejde a hra je u konce, vypise to
    ...
    def resize(self, *args):
        ...
        with self.canvas.before:
            ...
            for board_x, board_y in all_cells():
                BorderImage(pos=self.cell_pos(board_x, board_y), size=self.cell_size)
        if not self.b:
            return
